...
{
    public class RepositoryPessoaDapper : RepositoryBase<Pessoa>, IRepositoryPessoa
    {
        private readonly IUnitOfWork _uow;

        public RepositoryPessoaDapper(IUnitOfWork uow) : base(uow)
        {
            _uow = uow;
        }
        ...
        public override void Adicionar(ref Pessoa entity)
        {
            ...
            _uow.Connection.Execute(sb.ToString(), entity, _uow.Transaction);
            entity.Id = _uow.Connection.Query<int>(base.SequencialFB("seq_pessoa"), null, _uow.Transaction).First();
        }
        ...
        public IEnumerable<string> ListarEmails(int numeroEncontro, EnTipoExpositor tipo)
        {
            ...
            return _uow.Connection.Query<string>(sb.ToString(), null).ToList();
        }
        ...
        private IQueryable<PessoaPesquisa> RetornarPesquisa(EnTipoExpositor enTipoExpositor)
        {
            ...
            return _uow.Connection.Query<PessoaPesquisa>(sb.ToString(), null).AsQueryable();
        }
    }
}
