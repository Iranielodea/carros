...
namespace Carros.Dominio.Servicos
{
    public class ServicoPessoa : ServicoBase<Pessoa>, IServicoPessoa
    {
        ...
        private readonly IServicoEncontro _servicoEncontro;
        private readonly IServicoCadEncontro _servicoCadEncontro;
        private readonly IServicoSequencia _servicoSequencia;

        public ServicoPessoa(IRepositorioPessoa repositorioPessoa, IServicoEncontro servicoEncontro,
            IServicoCadEncontro servicoCadEncontro, IServicoSequencia servicoSequencia)
            : base(repositorioPessoa)
        {
            _repositorioPessoa = repositorioPessoa;
            _servicoEncontro = servicoEncontro;
        ...
        {
            var resultado = _servicoCadEncontro.RetornarTodos().FirstOrDefault(x => x.EventoAtual == "S");
            if (resultado != null)
            {
                var Encontro = _servicoEncontro.ObterNumeroFicha(idPessoa, resultado.NumeroEncontro);
                if (Encontro != null)
        ...
        public void Salvar(Pessoa model, EnTipoExpositor tipoExpositor)
        {
            ...
            if (model.Id == 0)
            {
                ...
                if (tipoExpositor == EnTipoExpositor.expCadSocio)
                    model.CodigoSocio = _servicoSequencia.IncrementarProximoNumero("SOCIO", "Cadastro de Sócios");
                if (tipoExpositor == EnTipoExpositor.expSocio)
                    model.CodigoFicha = _servicoSequencia.IncrementarProximoNumero("PESSOA", "Expositor do Clube");
                if (tipoExpositor == EnTipoExpositor.expVisitante)
                    model.CodigoVisita = _servicoSequencia.IncrementarProximoNumero("EXP_VISITA", "Expositor Visitante");
